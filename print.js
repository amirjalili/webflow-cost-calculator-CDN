$('#print').click(function(){
    ...
    item_list.forEach((value, index) => {
    	if(value != undefined){
       list_of_items += '<p class="list_item">' + (index +1) + '.' + value + '</p>';
    	}});
     item_list_area.forEach((value, index) => {
    	if(value != undefined){
       list_of_items += '<p class="list_item">' + (index +1 + item_list.length) + '.' + value + '</p>';
    	}});
    ...
    if ((item_list.length + item_list_area.length) > 6) {
    ...
    mywindow.document.write('<div id="list" style="margin-left: 50px;margin-right: 50px; height: 200px;">' + list_of_items +'</div>');
